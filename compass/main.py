...
def parseArgs():
    ...
    parser.add_argument("--latent-space", help="File with latent space reprsentation of samples for knn clustering or microclustering. "
                        "File must be a tsv with one row per sample and one column per dimension of the latent space.",
                        default=None, metavar="FILE")
    ...
    if args['generate_cache'] and \
        ...
        parser.error(
            ...
            "without --calc-metabolites" #Not sure about why this needs metabolites to be calculated
        )
